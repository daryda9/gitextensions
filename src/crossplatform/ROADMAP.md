...
| **Selettore di commit per il campo *From* del rebase** | **CHIUSO in M214.** `Views/ChooseCommitDialog` — la griglia vera in una terza istanza, limitata al branch corrente fino al merge base col bersaglio (`RevisionGridView.SetWalkBound`, nuovo). Il campo resta anche una casella di testo. Fuori: i due link ai genitori dell'upstream, e gli altri campi che upstream serve con lo stesso selettore (cherry-pick, archivio, confronto fra due revisioni). |
