...
Fonte di verità dettagliata: **`src/crossplatform/PORTING.md`** (milestone M1–M206,
...
| HEAD al momento dell'handoff | `7b84e9824` — **M204–M206: la chiusura dei residui che il collaudo aveva lasciato scritti** (tre unità, 2026-08-14). **M204 (`99e1c74b4`, preferenze di vista fra processi)**: la perdita di aggiornamento è stata **prima riprodotta in modo deterministico** con due processi veri che si danno appuntamento su file — A entra in `Update` e aspetta, B scrive una chiave **diversa** e finisce, A salva la copia caricata prima che B esistesse, e la scrittura di B sparisce senza un errore. Tre difese: un lock su file `.lock` **di lato** tenuto per tutto il carica-modifica-salva (di lato perché la scrittura finisce ora con una `rename`; su Linux `FileShare.None` è una `flock`, che **il kernel rilascia quando chi la tiene muore**, quindi niente lock stantìi né euristiche di scadenza); rilettura e merge **dentro** il lock, gratis perché `Update` era già un delta; e sostituzione atomica (prima era un nudo `File.WriteAllText`, quindi un processo morto a metà lasciava un file troncato, che si deserializza a default e **azzera in silenzio** tutto). La UI non si blocca mai: il lock è tentato **una volta sola e senza attesa**, quindi una scrittura non contesa resta in linea. Costi onesti: una mutazione differita gira più tardi, su un altro thread ed eventualmente **due volte**, quindi una delegata che chiude su una variabile **che cambia** ora si comporta male (undici call-site controllati, nessuno toccato, ma è un contratto nuovo); pochi millisecondi di esposizione a SIGKILL nel **solo** caso conteso; e nessuno chiama `Flush` alla chiusura. Banco `Tests/ViewPrefsRegression`: `PASS: 41 casi` in 6,6 s — 8 thread, 4 processi figli, 8822 letture concorrenti, 6 SIGKILL a metà scrittura; **non vacuo**, togliendo la sola difesa contro la perdita di aggiornamento `FAIL: 4 di 41` e si rompono esattamente quei casi. **Registrato e NON corretto**: tutti e sette gli archivi JSON del port condividono la scrittura non atomica, e sei su sette il carica-modifica-salva senza lock (`favorites.json`, `ui-state.json`, `app-settings.json`, `hotkeys.json`, `commit-info.json`, `scripts.json`, lista dei repository recenti); il peggiore è `ui-state.json`, che riscrive l'oggetto intero da un'istantanea presa all'avvio, quindi la finestra di perdita è **l'intera sessione**. **M205 (`2c5bcdf26`, `reword`/`squash` e l'uscita da un `am`)**: il pin che tiene il PTY dall'appendersi rispondeva anche all'editor di git prima che l'utente lo vedesse; il pin non può sparire (un `vi` ereditato appende il terminale — successo **tre volte** in questo port), quindi punta ora a un editor che **rifiuta** — git stampa «There was a problem with the editor 'false'», esce 1 e lascia il rebase **fermo** con l'indice pulito e il commit del passo già fatto: è il modo di fallire di git stesso a consegnare il momento giusto e il testo preparato. Fermarsi-e-chiedere e **non** raccogliere in anticipo, perché una coda consumata in lock-step è esattamente ciò che un rebase non garantisce (un conflitto a metà serie, un `--edit-todo` o uno `--skip` la desincronizzano e il testo finisce in silenzio sul commit sbagliato). Nessuna finestra nuova: è la casella multi-riga del reword della griglia, dentro il process dialog esistente. Annullare lascia il rebase fermo con tutti i pulsanti vivi, e la richiesta dichiara la cautela misurata: un `--continue` semplice successivo **non** riapre l'editor. `fixup` è lasciato stare perché è proprio lì che differisce — misurato, un rebase con `fixup` sotto un editor che rifiuta esce comunque **0**, git l'editor non lo apre. Pulizia `--cleanup=whitespace`, e la legenda di git la toglie `git stripspace --strip-comments`, che rispetta `core.commentChar` (provato con `core.commentChar=';'`, dove togliere i `#` a mano avrebbe cancellato una riga vera e tenuto la legenda). **Difetto di sicurezza preesistente trovato per strada**: git passa `GIT_EDITOR` a una **shell**, quindi il path dello script viene spezzato sugli spazi — sotto un TMPDIR con spazi e virgolette l'editor scriptato non partiva affatto e il rebase falliva senza niente da mostrare; tutti e tre i punti che usano un editor scriptato ora quotano per la shell, e tutto ciò che è variabile viaggia nell'ambiente. **`am`**: misurato, un `git am` semplice non lascia niente di unmerged (nessun conflitto, nessun rerere); solo `am --3way`, l'unica forma che il port esegue, produce stadi unmerged, dove `--continue` esce 128 e `--quit` lascia l'indice unmerged dietro di sé — il criterio è quindi l'indice unmerged, il banner aggiunge **un solo** `Resolve…` finché c'è unmerged e `ApplyPatchDialog` guadagna il «Solve conflicts» dell'upstream alla stessa condizione; riscritto il commento vecchio che dichiarava il port privo di un dialogo dei conflitti. Verificato con path contenenti spazi, uno aperto via symlink, e un messaggio con virgolette, trattino lungo, accenti, giapponese e una riga che inizia per `#`, intatti byte per byte. **Resta**: `MergeSessionService` fissa ancora `GIT_EDITOR=true`, quindi la richiesta del messaggio su `merge --continue` **non** è fatta. **M206 (`7b84e9824`, striscia delle schede)**: l'argomento sull'RTL è spezzato in due — il paragrafo resta LTR (un path è una sequenza letta da radice a foglia; dedurre la direzione allineerebbe un'etichetta RTL al bordo opposto rispetto alle vicine e rovescerebbe `مشروع/src` in un path che non esiste), ma questo non ha mai autorizzato la **posizione** dei puntini: il `…` è neutro, quindi in fondo a una riga LTR è disegnato alla destra visiva, che per arabo o ebraico è dove il nome **comincia** — misurato, i puntini stavano contro il pulsante di chiusura a marcare la prima lettera mentre la coda omessa usciva dall'altro bordo senza marcatura; ora sono scritti **prima** del testo tenuto quando la foglia inizia in RTL. **Vicolo cieco registrato**: avvolgere i segmenti RTL in isolate FSI/PDI **non fa niente**, il layout di Avalonia li ignora — dimostrato da due segmenti RTL adiacenti che si disegnano byte per byte identici con e senza; la stessa prova ha mostrato che il separatore **non** li scambia, quindi il pericolo che gli isolate dovevano prevenire qui non si presenta. Tooltip ed etichetta multi-segmento passano ora da `PathDisplay.CollapseHome`; il pulsante di chiusura ha un hover suo, da risorse vive della palette, controllato in entrambi i temi e nello stile Classic; `Squeeze` riscritto da scalatura proporzionale a un **tetto max-min** (si risolve un tetto perché i totali entrino, le schede sotto restano intatte), incrociato con 20 000 casi casuali, pavimento di 96 px mantenuto. **Deliberatamente invariato**: con nome RTL il numero del duplicato sta alla sua sinistra — bidi standard e corretto, letta da destra a sinistra la scheda dice «nome (1)». **Resta**: con RTL + duplicato + elisione i puntini finiscono, nell'ordine di lettura, **dopo** il `(1)`; due repository le cui etichette si riducono alla stessa coda restano indistinguibili al pavimento (preesistente); artefatto di arrotondamento sub-pixel al confine del tetto. Prima: `00175a9b2` (commit di documentazione) su `aebffd9e8` — **M196–M203: il giro di collaudo dei residui aperti, otto unità in parallelo** (richiesta esplicita: **provare** tutto ciò che era dichiarato «non coperto» o «non provato», invece di aggiungere altro; ogni unità con repository usa-e-getta e display Xvfb propri, con una regola sola — **misurare git, non ragionare su git**). **M196 (`7b8372768`, barra del sequencer)**: nello stato senza marcatore ma con l'indice ancora in conflitto — raggiunto senza toccare file (revert in conflitto → risoluzione → commit a mano → `update-index --unresolve`) — `--abort` esce **0**, stampa «Sembra che tu abbia spostato l'HEAD. Non eseguo il rewind», rimuove solo `.git/sequencer` e non tocca né commit né file, mentre il tooltip prometteva l'opposto **e** contraddiceva la propria conferma; lo stesso Abort lasciava l'indice unmerged senza dirlo; e il parser del verbo saltava righe vuote e commenti, mentre git legge la **prima** riga e un commento è un commento, quindi una todo che inizia con `#` faceva annunciare un cherry-pick che git nega. Verificato sano: worktree collegati con stato del sequencer per-worktree, senza contaminazione. **M197 (`ea84a9f07`, banco di prova della palette)**: nuovo `Tests/CommandPaletteRegression/`, `PASS: 10037 casi`, fuzz di 10 000 coppie in 137 ms; ha trovato che la corrispondenza allineava **unità di codice UTF-16**, così un allineamento poteva prendere la metà alta di una emoji e la metà bassa di un'altra e la finestra disegnava due metà di mojibake; banco dimostrato **non vacuo** (togliendo due delle quattro guardie: `FAIL: 267 of 10037`). Tre lacune dichiarate di M195 chiuse: spunta come «on»/«off» in colonna propria, voci di lingua ripristinate (la paura delle didascalie vecchie **non sopravvive alla prova**: la lista si ricostruisce a ogni apertura e la finestra si chiude prima del cambio) e motivo accanto alle righe grigie **solo dove è dimostrabile** — due esistono, nessuno dedotto. **M198 (`409cbc747`, dialogo di rebase)**: `--update-refs` non era passato in nessuna delle due forme, quindi decideva la configurazione e non c'era modo di rifiutarlo per un rebase solo; ora casella seminata dall'impostazione effettiva della repo che manda il flag **solo in disaccordo** con la configurazione (regola dell'upstream, `FormRebase.cs:331-335`), con una deviazione deliberata — chiave non impostata equivale a casella non spuntata, mentre l'upstream emette un `--no-update-refs` inutile a ogni rebase predefinito. Con «intervallo specifico» spuntato e un estremo vuoto il comando rifaceva in silenzio **tutto** il branch: adesso una nota agganciata all'anteprima lo dichiara. Verificato sano: un rebase conflittuale avviato dall'**albero di sinistra** pre-compila bene e le opzioni arrivano davvero a git; `--autosquash` accorpa davvero, e la misura importante è che coinvolge **entrambi** gli editor (`GIT_SEQUENCE_EDITOR` **e** `GIT_EDITOR`) — con nessuno dei due fissato git lancia vi e muore, il port li fissa entrambi, verificato lanciandolo con le due variabili **non impostate**. **M199 (`004d3610d`, diff di immagini)**: la modalità differenza restava appesa per sempre su un PNG grigio a **16 bit** (Skia dà una superficie che Avalonia non sa descrivere, `CopyPixels` lancia, il pannello restava sullo spinner con l'unica traccia in console); un **contenitore** era spacciato per l'intero file — un `.ico` da 6 voci mostrava una dimensione, una GIF da 4 fotogrammi il primo, in silenzio, e l'icona **troncata** dimostra che conta perché scendeva a 64×64 leggendosi come un ridimensionamento; due PNG a 16 bit diversi nel byte basso di ogni pixel decodificavano identici, «0 di 4096 pixel differenti» accanto a «+1 B». **Nota vecchia da correggere: Skia sceglie la voce ICO più GRANDE, non la prima — l'affermazione che un ICO degradi sempre a 16×16 era falsa.** Verificato sano: BMP RLE4/RLE8 esatte al pixel, GIF interlacciata corretta, JPEG progressiva esatta, JPEG CMYK **non** invertita. **Non corretto, la lacuna più grossa rimasta**: un PNG/GIF/WEBP/BMP **troncato** disegna mezza immagine senza avviso — segnalarlo richiede il codice di risultato incompleto di `SKCodec`, e SkiaSharp non è dipendenza diretta del progetto App. **M200 (`8659f1868`, editor della todo di rebase)**: il riepilogo calcolava i commit persi come **righe** rimosse, quindi cancellare un solo `exec` annunciava «1 commit non sarà nel branch ricostruito» — la frase su cui l'utente agisce; `reword` e `squash` erano promessi dalla legenda e consegnati da nessuno dei due, perché `Continue` fissa `GIT_EDITOR=true` (che è ciò che impedisce alla PTY di appendersi, e quella trappola **è** chiusa) ma risponde all'editor prima che l'utente lo veda, quindi reword lasciava il messaggio invariato e squash dava la concatenazione predefinita di git: la legenda ora lo dice e rimanda a «Reword commit…», mentre la correzione funzionale tocca il **chiamante** di `Continue` ed è **riportata, non tentata**. Il discriminatore `Interactive` regge, ma la sua documentazione nominava **un** falso negativo dove ce ne sono **due** (`-i --empty=drop` e `--rebase-merges` senza `-i`); entrambi sbagliano verso l'offrire meno. Verificato sano: tutti e sei i verbi esotici (`exec`, `break`, `label`, `reset`, `merge`, `update-ref`) si disegnano interi e tornano **byte per byte**, controllato sia dal parser (17 righe identiche) sia da un capo all'altro attraverso la finestra; riordinare e cancellare lascia una todo che git esegue. Pericolo che è **di git, non del port**: cancellare un `label` nominato da un `merge` successivo viene accettato in silenzio e muore dopo, e non si può rimetterlo perché nessun passo può essere aggiunto. **M201 (`32ef95531`, rerere nei worktree collegati)**: il bug vero — in un worktree collegato `MERGE_RR` è per-worktree ma `rr-cache` vive **solo** nella common directory, e il port chiedeva `--absolute-git-dir` per entrambe. Riprodotto: git riapplicava una risoluzione mentre l'app non mostrava banner, casella non spuntata, nessun pulsante della cache e nessun avviso — cioè esattamente il replay silenzioso che questa funzione esiste per rendere visibile. Inoltre il testo di cherry-pick/revert/am diceva «la prossima volta», ma misurato dentro **un solo** `cherry-pick master..topic` il passo 1 registra e il passo 2 della stessa esecuzione riapplica. Una nuova sonda dell'operazione legge i marcatori per-worktree, che sono disgiunti; guida **solo** il testo di rerere, perché le etichette ours/theirs non devono capovolgersi (HEAD resta *ours*). Misura che delimita: `git am` semplice non coinvolge rerere per niente — solo `am -3`. Verificato sano: un submodule a metà rebase risolve su `modules/<nome>` e il port legge la cache giusta; l'annuncio guardato di «già risolto» sopravvive a un merge che porta insieme conflitti binari, di soli permessi, delete/modify, rename/rename e symlink. **Lacuna nota**: il ramo `am` del testo nuovo è **irraggiungibile**, nessun ingresso offre un dialogo dei conflitti durante un `am`. **M202 (`8cea6b14e`, striscia delle schede)**: il numero del duplicato era incollato in fondo all'etichetta e l'elisione taglia dalla fine, quindi veniva tagliato per primo e due schede sulla stessa repository sembravano identiche, a **qualunque** larghezza; la scrollbar ad auto-nascondimento di Fluent sovrappone il contenuto — innocua su un pannello alto, qui copriva da parte a parte una riga di 24 px, proprio quando avere più schede di quante entrino rende importante leggerle. Terzo, latente e onestamente **non dimostrato**: il taglio di ultima istanza lavorava su indici UTF-16 grezzi e, siccome la ricerca prende il candidato **più lungo che entra**, uno tagliato a metà è più stretto e quindi preferito — nessun artefatto visibile riproducibile (il surrogato orfano si disegnava come niente nello stack di font di prova), corretto lo stesso. Verificato sano: tema chiaro in ogni stato, cambio dal vivo in entrambe le direzioni senza pennelli vecchi, entrambe le dimensioni della UI compreso il pavimento di 96 px a 1,25×, numerazione dei duplicati con una scheda chiusa in mezzo, CJK, path profondi, 14 schede. **Lasciato lì con la prova accanto**: l'arabo mette i puntini sulla destra visiva; il tooltip mostra il path assoluto grezzo invece della forma con `~`; il pulsante di chiusura non ha hover suo; a budget esaurito è la scheda **più lunga** a tenersi la larghezza naturale. L'opzione tema **System non è stata esercitata** — il suo portal XDG non esiste sul display di prova. **M203 (`aebffd9e8`, didascalie del diff)**: «Confronta il file con la cartella di lavoro» redirige entrambi i lati ma le didascalie restavano su commit-contro-genitore — un file **aggiunto** diceva «non esiste» mentre ne mostrava il contenuto, un **cancellato** nominava una revisione che `cat-file` dichiara assente, e un **rinominato** teneva il path vecchio con il pannello vuoto; la selezione **multi-revisione** era peggio del previsto: con tre sezioni da due tip divergenti, un file sotto «Diff BASE with A» prendeva il **blob** di sinistra dal tip A e la **didascalia** di destra dal tip B. Le righe che portano la propria coppia risolvono ora da quella coppia, come già faceva il caricatore della patch; nessuna sonda git aggiunta e nessun codice asincrono nuovo. **Verificate senza scrivere codice**: l'editor di merge oltre il budget di allineamento (conflitto 10 000 × 10 000 righe aperto in ~1 s; oltre `MaxEdits = 1200` il riepilogo dell'auto-merge diventa una frase onesta invece di un numero; con lunghezze diverse **nessuna** marcatura intra-riga invece di marcature indovinate; «Resolve trivial (N)» resta vero perché il suo classificatore non ha budget proprio) e due finestre di merge insieme (irraggiungibili in un processo solo perché la finestra è modale, quindi servono due istanze; lettura all'apertura, ultimo che scrive vince, nessuna interferenza fra i contatori — giudicato difendibile e lasciato stare). **Pericolo separato registrato**: `ViewPrefsService.Update` è carica-modifica-salva **senza lock fra processi**, quindi due istanze possono perdere una preferenza che non c'entra — **preesistente, non corretto**. Prima: `d31fbf690` — **M194–M195: la storia si cerca per contenuto, e ogni voce di menu si raggiunge da tastiera** (le prime due voci del §2.1 della roadmap). **M194 (ricerca per contenuto)**: il motore c'era già — `RevisionFilter.DiffContent` → `git log -S`/`-G` — ma ci si arrivava solo scrivendo nella casella della griglia, e la casella non lasciava scegliere fra le due forme. La differenza è stata **misurata** su un repo usa-e-getta (aggiunta → spostamento → rimozione → riaggiunta): `-S` dà 3 commit, `-G` ne dà 4, e il quarto è quello che ha solo **spostato** la riga — è tutta lì la differenza fra le due domande. Ora la scelta letterale/regex esce dal dialogo del filtro avanzato ed entra nella tendina della casella, visibile **solo** mentre «Diff contains» è armato (`Diff contains (text)` / `(pattern)` sul pulsante e sul segnaposto); nuova voce **«Search history for …»** nel menu contestuale del diff sulla selezione, che toglie il `+`/`-`/spazio iniziale se la selezione parte dalla **colonna 1** — quella colonna è formato del patch, non contenuto del file — arma la forma **letterale** e su selezione multiriga usa la prima riga dicendolo nell'etichetta; riavviare una camminata **annulla** la precedente col token che `LoadRevisionPage` già accettava. **Non coperto**: blame e vista del contenuto non hanno la voce (nessuna selezione carattere per carattere lì), l'annullamento è osservato **fra un blocco di output e l'altro** quindi una camminata muta per minuti non muore subito, e non c'è un pulsante «annulla» visibile. **M195 (palette dei comandi, Ctrl+Shift+P)**: feature **INEDITA**, upstream non ha niente di equivalente. La lista **è il menu vero**, percorso all'apertura — `MainMenu.EnumerateCommands()` cammina `_topLevel` e non `_bar.Items`, così le voci finite nell'overflow «…» non sono né perse né doppie — quindi la disponibilità (niente repository, niente selezione) la decide il menu che già la calcola, e un elenco parallelo scritto a mano non c'è da tenere in vita. Aggiunti anche i comandi legati a un tasto ma assenti da ogni menu (famiglia `Focus*`, `GoToParent`/`Child`, varianti rapide di pull/push) sotto un percorso «Keyboard». Corrispondenza a sottosequenza con punteggio (inizi di parola, tratti contigui, etichetta foglia sopra il percorso) e caratteri trovati evidenziati; recenti in cima, salvati per **id non tradotto** perché la didascalia si perderebbe al cambio di lingua. Comandi non disponibili **in grigio, non nascosti**. **Esc non può invocare**: la scelta è scritta solo dal percorso di accettazione, la disciplina del selettore submodule. La finestra si chiude **prima** che l'azione parta, perché molte azioni aprono una modale della finestra principale. Difetto trovato provandola: `VirtualizingStackPanel` richiama il template con un elemento **null** mentre ricicla, e l'app cadeva alla prima lettera. **Deviazione dall'originale**: `Ctrl+Shift+P` è `QuickPull` in upstream — la palette se lo prende e `QuickPull` passa a **Shift+F8**, accanto a `QuickPullOrFetch` (F8); rimosso anche l'array `GesturePriority`, che senza collisione non difendeva più niente. **Non coperto**: nessun motivo accanto alle righe grigie, le voci Vista → Lingua non offerte, le opzioni a spunta offerte ma senza lo stato della spunta disegnato, e **nessun test automatico** benché il motore di corrispondenza sia isolato apposta. Entrambe verificate a schermo sotto Xvfb con configurazione isolata; build `--no-incremental`: `Avvisi: 0 / Errori: 0`). Prima: `91d14d4e5` — **M192–M193: un revert o cherry-pick fermo si chiude dall'app** (segnalato: revert fatto, conflitto risolto, e la barra restava su «A revert is in progress» senza offrire **niente** — l'app nominava il comando git e mandava in un terminale. Il buco era voluto e scritto: mancava il servizio. **M192**: `SequencerSessionService`, uno solo per revert e cherry-pick perché git li implementa con **un** sequencer; `--abort` e `--quit` **misurati** su stati identici (abort toglie i commit già fatti e rimette i file, quit tiene tutto e dimentica la sequenza) e messi in parole perché la differenza si legga; `Skip` assente su un'operazione a un commit solo, dove è `abort` sotto altro nome. Trappola dell'editor per la **terza** volta: `Continue` committa, quindi apre `core.editor`, e scatta **solo su PTY** — verificato con uno script che registra ed esegue `vim`. Upstream non ha questi comandi: lavoro **originale**, dichiarato. **M193**: la barra li trovava solo dai marcatori `CHERRY_PICK_HEAD`/`REVERT_HEAD`, che git cancella appena il passo viene committato a mano mentre `.git/sequencer/todo` tiene il resto — vicolo cieco riaperto da un'altra porta. Ora barra **e** pulsanti ricadono sul sequencer, con **un solo parser** condiviso; e tre frasi mentivano: il contatore conterebbe due volte il passo appena committato (soppresso), `--skip` da lì fallisce (pulsante tolto), `--abort` rifiuta di riavvolgere un HEAD spostato ed è `--quit` sotto altro nome). Prima: `8e43a7574` — **M190–M191: il rebase esce dal silenzio (parità con `FormRebase`)** (chiesto dall'utente dopo aver visto che nell'originale il rebase apre una finestra e qui partiva diretto). **M190**: il port lanciava `git rebase` da una voce di menu portandosi dietro il solo autostash globale — l'ingresso più pericoloso era il più silenzioso. Ora una finestra con `Interactive`, `Autosquash`, `Auto stash`, `Rebase merges`, le due opzioni sulle date, il ref **onto** e un **intervallo**, con le esclusioni reciproche di upstream riprodotte e non reinventate; una spunta che si disabilita viene anche **tolta**, così l'anteprima del comando non mente (upstream la lasciava spuntata). **`--preserve-merges` non copiato**: git 2.43 risponde `fatal: --preserve-merges was replaced by --rebase-merges`, quindi l'etichetta di upstream nomina un flag che qui non gira. Autostash diventa scelta per singolo rebase. Tutti e **tre** gli ingressi (pannello branch, griglia, albero) aprono la stessa finestra; il vecchio `RebaseOnto` silenzioso rimosso. **M191**: `Edit todo` dalla barra — round trip via `GIT_SEQUENCE_EDITOR` scriptato (la trappola dell'editor su PTY di M183), con git che fornisce la presentazione e **valida lui** la lista. Ha corretto un'assunzione sbagliata del port: git scrive `rebase-merge/interactive` **anche per un rebase normale**, quindi ogni rebase col backend di merge risultava interattivo; il marcatore che discrimina è il fratello `drop_redundant_commits`. La conferma sulla lista svuotata dice il fatto **misurato** e non la legenda di git: a metà rebase non annulla, ferma dov'è e tiene ciò che è già stato rifatto). Prima: `de5aac366` — **M185–M189: chiusi i debiti piccoli, e l'audit ne ha trovati due veri** (cinque milestone, unità parallele su file disgiunti). **M185**: `InlineDiff` non aveva **nessuna** suite; scriverne una ha trovato due difetti alla prima esecuzione — testo **spostato** riportato come «niente cambiato» (`Refine` tosava testa e coda come se gli span stessero agli stessi offset; 111 hit su 4000 nel fuzz, tocca ogni riordino di token) e **confine di span dentro una surrogate pair** (le guardie di coda testavano `IsHighSurrogate` dove la forma pericolosa è una **low**). Corretti verificando la premessa vera di `Refine` («tutto ciò che sta fuori dagli span è già identico») invece di assumerla, e con un helper unico per tutti i confini. Suite con invarianti + fuzz a seed fisso, esteso a 70 000 coppie: zero violazioni, nessuna quarantena. **M186**: modalità intra-riga persistita **per nome, non per indice**, e marcature nel pannello del risultato con una regola sola per tutti e quattro i pannelli. **M187**: rerere provato finalmente su **rebase** — e scoperto che «rerere ha già risolto questi» **mentiva** anche nel merge, perché un conflitto binario o di sole permessi non ha marker; corretto, e i testi ridimensionati sul fatto misurato (il rebase da manuale con tre commit sulla stessa riga **non** riapplica niente: riapplica lo stesso conflitto che **ricorre**). **M188**: firma ICO stretta — nella forma vecchia coincideva col picture start code MPEG. **M189**: eliminato il rilevatore di immagini **gemello** invece di tenerlo in sincrono, ed etichette che per un file aggiunto non nominano più una revisione inesistente, corrette alla fonte. Build `--no-incremental`: `Avvisi: 0 / Errori: 0`; suite `Tests/InlineDiffRegression`: 10 028 casi, exit 0). Prima: `88a2e0d4c` — **M184: una scheda lunga perde il percorso, mai il nome della repo** (segnalato: `pluma_orchestrator/ai-server/core/…`, spariva proprio il nome. L'ellissi di fine riga è la regola sbagliata per un percorso: la coda porta l'identità, la testa è contesto. Ora il degrado va **dalla testa verso l'interno**, un segmento alla volta, perché i segmenti vicini alla foglia sono quelli che la collocano; la foglia si taglia solo come ultima risorsa. Niente iniziali `p…/a…/`: un'iniziale si legge come un nome e invita a espanderla. Tooltip sempre col percorso completo. Il livello è **misurato** con lo stesso typeface/size/weight/style del disegno, non contato a caratteri, e ricalcolato a ogni cambio di larghezza. **Due modifiche di layout erano indispensabili**: uno `StackPanel` orizzontale misura i figli con larghezza infinita, quindi la riga della scheda è un `Grid`, e la striscia ripartisce la larghezza vera fino a un pavimento — senza budget restringere la finestra non stringeva mai una scheda e i livelli corti erano irraggiungibili. Verificato da 1300 a 380 px; pastiglia, riordino, chiusura e persistenza intatti). Prima: `62aac6bcc` — **M183: un pull che deve fondere non aspetta più un editor** (segnalato: Pull - merge fermo su «Running…» con il template del messaggio di merge storpiato a schermo; Abort + Continue lo chiudeva subito. Causa: git apre `core.editor` quando stdin e stdout sono **lo stesso terminale**, ed è la forma della finestra di processo, che gira i comandi interattivi su **PTY** — quindi git credeva ci fosse un umano e avviava un editor a schermo intero dentro una casella che terminale non è. Corretto con `--no-edit` di `pull` e non con `GIT_EDITOR=true`, perché il flag si vede nella casella «Command to be executed»: un messaggio di merge che l'utente non ha potuto scrivere è una decisione che la riga di comando deve confessare. Audit di tutti i comandi che possono aprire un editor — `merge`, `revert`, `cherry-pick`, `tag -a`, `notes`, `am`, `commit`, `rebase` e le continuazioni: già tutti protetti, `pull` era l'unico buco. **Regola da tenere: un comando lanciato sulla PTY deve essere esplicitamente privo di editor.** Scoperto e non corretto di proposito: la finestra di processo non può accorgersi di essere bloccata su un editor. Riprodotto prima, verificato dopo; conflitto / fast-forward / già aggiornato invariati). Prima: `1a6eff3f2` — **M182: quello che git ha fuso da solo si vede, e si può scavalcare** (segnalato con lo screenshot di kdiff3: l'editor non diceva quanti conflitti erano stati risolti da soli. Stavamo facendo il lavoro e lo nascondevamo — `merge-file` fonde in silenzio, quindi un file con 8 modifiche e 1 conflitto si apre sembrando averne una. Conteggio **dedotto**: si rimettono i conflitti **alla base**, si diffa contro la base, quel che resta è ciò che git ha deciso da solo; attribuzione **per prova** con base↔LOCAL e base↔REMOTE, e un tratto che nessuno rivendica viene scartato invece che indovinato. Myers a budget in memoria, non tre processi git su un percorso che deve sembrare istantaneo; a budget esaurito dichiara il conteggio sconosciuto. Riga di riepilogo, **non** una modale da chiudere. Fusioni automatiche ancorate, marcate `AUTO ← LOCAL/REMOTE/= both` e navigabili: una fusione automatica sbagliata è più pericolosa di un conflitto perché nessuno la guarda. Secondo punto: **tasto destro** sulla regione sotto il puntatore, nel risultato e nei tre pannelli, voci radio spuntate dallo stato riletto dal testo, funzionante anche sulle fusioni automatiche con ritorno alla risposta di git e marcatura `OVERRIDE`; offerti solo i lati che metterebbero caratteri davvero diversi. Fixture a numeri noti: 8/6/2 attesi = ottenuti). Prima: `9ee2cafed` — **M177–M181: chiuso il §1 della roadmap, indipendenza dagli strumenti esterni** (cinque milestone in una tornata, orchestrate su unità parallele con file disgiunti). **M177 diff intra-riga**: era il vero divario residuo con kdiff3 — `InlineDiff` risponde in memoria (`--word-diff` sarebbe un processo per coppia di righe), token a parola, tetto di celle contro le righe minificate, e se cambia più di metà di entrambe le righe **non evidenzia niente**; ~8,6 µs a riga, solo righe visibili, `IBackgroundRenderer`. **M178 editor di merge**: conflitti banali (solo spaziatura, spazi in coda, fine riga, righe vuote, un lato immutato) chiusi con un clic ma **mai all'apertura** e reversibili uno per uno; gli spazi si collassano, mai si cancellano (in Python l'indentazione **è** il programma); marcature intra-riga LOCAL↔REMOTE più la modalità ogni-lato↔BASE; rifiuti **tipizzati**. **M179 diff immagini**: affiancate/sovrapposte/differenza, riconoscimento **dai byte** e non dall'estensione, banner sopra la patch inutile. **M180 `git rerere`**: misurato su git 2.43 — è attivo anche solo perché esiste `rr-cache`, dopo un replay `status`/`remaining`/`diff` sono vuoti **mentre l'indice è ancora unmerged**, e `forget` fuori da un merge si auto-annulla. **M181 conflitti non fondibili**: selettore del commit per i submodule (mostra cosa c'è **in mezzo**, e scrive anche un **terzo** commit con `update-index --cacheinfo 160000`, cosa che `checkout --ours` non sa esprimere), pannello guidato al posto del pulsante grigio, rerere in UI con «già risolti» **dedotto dall'albero di lavoro** perché dopo un replay git tronca `MERGE_RR`. Difetto trovato e chiuso in corsa: `Escape` scavalcava Annulla nel selettore submodule. Build `--no-incremental`: `Avvisi: 0 / Errori: 0`. Prima: `897dfd07f` — `897dfd07f` — **M176: pastiglia del checkout sulle schede** (due cloni dello stesso progetto hanno gli stessi submodule e le schede si confondono. Il nome della cartella padre c'era già — `BuildLabels` allunga l'etichetta finché i percorsi si separano — ma la differenza cade **in mezzo** a due stringhe identiche: si legge, non si vede. Ora 3px di colore per checkout, uguale su tutte le schede dello stesso clone; il checkout è il working tree più esterno sopra il percorso, trovato risalendo gli antenati in cerca di un `.git` **file o cartella** perché quello di un submodule è un file. Spenta con un solo checkout aperto. Verificato: blu su `work`, ambra su `review`, pastiglie assenti con un clone solo). Prima: `b2977ecc1` — **M175: la scelta nel merge è uno STATO, non una modifica a senso unico** (segnalato usando lo strumento: preso un lato non si potevano più scegliere le alternative. Difetto di progetto: la sostituzione del blocco di marker **distruggeva la regione**. Ricerca prima del codice: gli strumenti affermati rendono la decisione **rivedibile** e mettono il controllo **dov'è il conflitto**. Ora ogni conflitto è una `Region` viva per tutta la finestra, delimitata da due `TextAnchor` (`BeforeInsertion` sull'inizio, `AfterInsertion` sulla fine) che il documento sposta da solo; scegliere riscrive fra le ancore, riscegliere lo stesso lato rimette il conflitto. La scelta è **dedotta** dal testo fra le ancore, quindi digitare dentro una regione la marca EDITED senza contabilità. Aggiunti: margine con stato per conflitto, toggle che riportano lo stato, salto al prossimo indeciso, All LOCAL/REMOTE sui soli indecisi, Restore conflict, contatore risolti/totale, cursore che sceglie il conflitto attivo). Prima: `67270c30c` — **M174: difftool affiancato INTERNO** (secondo mattone dell'indipendenza dopo il M172. «Compare side by side…» nel menu contestuale dei file, sopra «Open in external difftool» che resta intatto; **non serve `diff.tool`**, quindi su una macchina senza niente configurato è l'unica voce che fa qualcosa. Il diff è di git: `git diff --no-index -U0` sulle due versioni, si leggono solo gli header di hunk e da quelli si costruisce l'allineamento — `--no-index` perché è lo stesso percorso qualunque sia il lato (commit, working tree, o niente per un file aggiunto/cancellato) e tiene fuori il problema dei rename. Dentro l'hunk i lati sono appaiati riga per riga, così una riga cambiata sta di fronte a quella da cui è cambiata. **I numeri di riga vengono dall'allineamento**: i pannelli sono imbottiti per restare allineati, quindi la numerazione dell'editor conterebbe l'imbottitura; su una riga fantoccio non si scrive niente invece di inventare un numero. `ResolveSide` estratto e condiviso con «Copy old/new version». **Due difetti trovati provandolo**: la finestra non aveva via d'uscita (né pulsante né Escape — aggiunto Close con `IsCancel`), e un file vuoto veniva spezzato in una riga vuota invece che in nessuna, con una finta «riga vuota rimossa» sotto le aggiunte. Verificato: 8 differenze su `MainWindow.cs` con scorrimento sincronizzato, file aggiunto senza righe fantasma, Escape chiude, zero eccezioni). Prima: `373619bb0` — **M173: collaudo dell'editor di merge su conflitti veri** (il M172 era provato solo su un fixture sintetico di 40 righe; qui su clone usa-e-getta del repo: `MainWindow.cs` 5431 righe con 8 conflitti fra cui riga 1 e ultima riga, `PORTING.md` 8502 righe UTF-8, un file CRLF con `* -text`, uno senza newline finale. **Due difetti trovati e chiusi**: (1) la newline finale aggiunta di sua iniziativa — il flag veniva letto dall'output di `git merge-file`, che termina sempre la sua uscita perché il `>>>>>>>` vuole una riga sua; ora si legge dal nostro blob, e risolvendo LOCAL ovunque `git status` è pulito cioè il file è byte per byte quello locale; (2) un `=======` isolato scambiato per detrito, mentre è un heading setext Markdown valido — ora conta solo se c'è altro detrito accanto. **Misura che ha smentito la mia ipotesi**: 300 battute si assestano in 3,7 s su 5431 righe e 3,1 s su 40, quindi la dimensione vale ~2 ms a battuta e la scansione non era il collo di bottiglia; riscritta lo stesso come assicurazione, dichiarata tale. Esito: 4 file risolti, zero marker residui, accenti intatti, 60 CRLF e 0 LF isolati, zero eccezioni). Prima: `d978b46c7` — **M172: editor di merge a tre vie INTERNO** (richiesto: «renderci indipendenti da software esterni come kdiff3». **Feature INEDITA §4**: l'upstream lancia solo `git mergetool`. Lo strumento esterno resta intatto, il pulsante **Merge** apre l'editor del port. **Il merge resta di git**: i tre stage vanno a `git merge-file --diff3`, `MergeToolService` aggiunge solo la struttura (chunk tipizzati dai blocchi di marker) — riscrivere diff3 avrebbe messo accanto a git una seconda risposta diversa per sempre. **Il documento È il modello**: il pannello del risultato tiene i marker veri, ogni pulsante riscrive il blocco su cui sta e il contatore si ri-deriva scansionando dopo ogni battuta, quindi modifica a mano e pulsanti convivono senza riconciliazione. Buco trovato provando a mano e chiuso: rompendo il solo marker di apertura la finestra diceva «tutti risolti» su un file ancora pieno di `=======`/`>>>>>>>` → `CountStrayMarkers` li conta e lo dice. Verificato a schermo: take LOCAL/REMOTE/both, i tre pannelli che si riposizionano, salvataggio con `ls-files --unmerged` vuoto e newline finale conservata, e **con `merge.tool` vuoto i due pulsanti esterni sono disabilitati mentre Merge funziona** — tutto il punto dell'indipendenza). Prima: `04b8a0cff` — **M171: il primo clic su un commit mette a fuoco la griglia** (segnalazione con due screenshot, «la prima volta la linea non ha i contorni bianchi». Campionati: primo clic `#2F57AA` senza alone, secondo `#215BDD` con rettangolo `#BACCF4` — sono la selezione **inattiva** e **attiva**, quindi la griglia dopo il primo clic non aveva davvero il focus. **Causa: il rebind, non il clic** — il clic mette a fuoco la `ListBoxItem`, la selezione cambia l'autore evidenziato, l'autore ri-templatizza tutte le righe e **distrugge i contenitori**, quello a fuoco compreso. Dimostrato dai due casi che lo incastrano: seconda riga con lo **stesso autore** → focus al primo clic; **Esc** (azzera l'autore) → il clic dopo torna senza focus. **Il ripristino puntava al controllo sbagliato**: una `ListBox` non è focusabile, lo sono le sue item, quindi `_list.Focus()` era quasi un no-op ovunque. Nuovo `FocusSelectedRow` (contenitore della riga selezionata, ripiego sulla lista); ci passa anche `SelectIndex`, differito a `Loaded`. **Chiude anche le due anomalie annotate nel M170**: frecce che non muovevano niente e type-to-search che non partiva — stessa radice. Verificato a schermo, precedenza dell'Esc sulla ricerca rapida finalmente **esercitata**. Trappola del banco: la ricerca rapida scade a 4 s, i tasti vanno inviati senza pause). Prima: `d03b76c23` — **M170: Esc lascia andare il commit selezionato** (richiesto esplicitamente, e **dichiarato feature INEDITA** per la regola del §4: l'originale non lo fa — il suo Escape nasconde solo il tooltip e il commit si lascia con Ctrl+clic. Messo **dopo** il ramo della ricerca rapida sullo stesso tasto, garantito dall'ordine della catena `else if`: mentre una type-to-search è in corso l'Escape appartiene alla ricerca, come upstream. `ClearSelection` assegna `SelectedIndex = -1` e non svuota `SelectedItems`, che alzerebbe un cambiamento per riga. Verificato a schermo. **Nota d'ambiente per ogni prova da tastiera futura**: sotto Xvfb senza window manager il focus X è `PointerRoot` e i tasti non arrivano al controllo cliccato finché non si fa `set_input_focus` sulla finestra — senza quel passo l'Escape sembrava non fare niente. **Osservazione non corretta qui**: col focus giusto, `Down` non sposta la selezione e digitare non avvia la ricerca rapida — può essere `SelectionMode.Multiple` o un difetto vero, da guardare a parte). Prima: `ef844c00b` — **M169: lasciare andare un commit non porta più giù l'app** (segnalazione «non ho più la possibilità di deselezionare la commit; nell'originale Esc deseleziona?». **La domanda, verificata nel sorgente**: no — l'Escape di `RevisionGridControl:914` nasconde solo il tooltip, quello di `QuickSearchProvider:102` annulla la ricerca; nel Windows si lascia con **Ctrl+clic**, comportamento standard di un `DataGridView` multi-selezione. **Il fatto**: nel port Ctrl+clic non era rifiutato, **crashava** — `Cannot change source while update is in progress`, stessa forma fatale del M157 un anello più in là. Svuotare la selezione cambia l'autore evidenziato → ri-templatizza tutte le righe → assegna `ItemsSource`, **dentro** il batch del `SelectionModel` da cui Avalonia alza `SelectionChanged`; la guardia `_rebinding` conosceva solo i batch aperti da questa vista, non quelli aperti dal puntatore. Nuovo `_inSelectionChanged`, `RebindRows` rimanda alla passata differita che già aveva. **Seconda metà**: i tre eventi esistenti portano tutti qualcosa, quindi «niente selezionato» era uno stato che l'host non poteva sapere e i pannelli continuavano a descrivere il commit lasciato — nuovo `SelectionCleared`, `MainWindow` risponde col `ResetBottomPanes` del cambio repository. Verificato a schermo, zero eccezioni. **Escape NON aggiunto**: sarebbe una feature inedita, e la regola del §4 è che non parte di iniziativa mia). Prima: `e6f8ef759` — **M168: una striscia di schede sta sulla superficie a cui appartiene** (segnalazione con screenshot, «rendi tutto flat»: misurato, la striscia delle schede di pannello stava su `App.Window` `#141518` e tutto ciò che sta sotto su `App.Panel` `#1C1D21` — uno scalino intero; la striscia delle schede di repository aveva la stessa cucitura al contrario, `App.PanelAlt` sopra un corpo `App.Panel`. Entrambe dipingono ora `App.Panel`. Non si perde niente: dal M166 la scheda selezionata è marcata da barra accento + inchiostro + peso, e la scheda di repository attiva tiene la sua riga accento sul bordo superiore. L'hover delle schede si deriva ora da `App.Panel`, altrimenti l'eco del puntatore sarebbe uno scalino **sotto** il terreno su cui atterra. Verificato a schermo scuro e chiaro: striscia e corpo campionano lo stesso valore). Prima: `d9e3b0a82` — **M167: la cucitura fra due pannelli ha un colore suo, più sottile** (seguito del M166 con uno screenshot di VS Code. Nel M166 avevo scritto che il colore era «già giusto» a 1,58:1 contro il ~1,5 di VS Code: **quel ~1,5 era a memoria**. Misurato sullo screenshot: la linea vera è 1 px `#2B2B2B`, **1,16:1** sull'editor e **1,25:1** sul pannello — la mia era mezza volta più forte. Ora due token, come li tiene il riferimento: **`App.Rule`** è la cucitura dentro una finestra, **`App.Border`** resta il bordo di una cosa (carta di un flyout, scatola di un gruppo, contorno di un campo). `App.Rule` = `#2A2B32` / `#E8E8EC`, **1,19:1** e **1,20:1** sul pannello, verificati campionando i pixel resi. Classic tiene `App.Rule` = `App.Border`: la sua toolbar è più **chiara** della riga assottigliata, quindi lì sparirebbe invece di attenuarsi. 25 punti strutturali spostati + l'ultimo splitter dipinto rimasto, quello del log dei comandi. **Le caselle di testo no**, di proposito — il loro contorno è l'unica cosa che dice dove si scrive; conseguenza: ora sono le linee più forti sullo schermo, e assottigliarle è una decisione a parte). Prima: `3972e4e46` — **M166: una sola lingua per la chrome, e meno linee a disegnarla** (richiesta con due screenshot, il proprio e uno di VS Code. Il colore delle linee era **già giusto** — `App.Border` sta a 1,58:1 sul pannello scuro, dove sta quella di VS Code: erano il **numero** delle linee, lo **spessore** e **tre copie divergenti** della stessa barra. Diff e lista dei file portavano una copia privata di `BarButtonStyles` con hover su una superficie **più scura** della barra e aggancio `App.Selection` + contorno `App.Accent`; l'albero non ne usava nessuna, quindi i sei filtri cadevano su Fluent — **sei scatole blu sature**. Ora tutte e tre la condividono, e gli stati si distinguono per il **solo riempimento**. Via le linee doppie sotto la toolbar della lista dei file (capelli paralleli a 3 px, misurati); i quattro `GridSplitter` che dipingevano `App.Border` su 4 px di presa — quattro volte ogni altra linea dell'app — diventano presa trasparente + riga da 1 px, nuovo `Theming/PaneSplitter`; la scheda selezionata perde riempimento blu e contorno accento e resta barra accento (4,96:1 scuro / 6,05:1 chiaro) + inchiostro + peso. Classic e le icone bicolori **non toccate**, di proposito. Verificato a schermo, scuro e chiaro). Prima: `56b4242ef` — **lista delle feature INEDITE su richiesta** (aperta in PORTING §4 e HANDOFF §4: nessuna voce parte di iniziativa propria, e quando si consegna va dichiarata inedita perché non c'è un comportamento upstream a cui appellarsi. Prima voce: fondere il submodule dal superprogetto). Prima: `d4a2a360e` — **M165: i conflitti di puntatore dei submodule si risolvono dalla parte che scegli** (domanda dell'utente; la risposta era «no, e peggio di no»: un gitlink non ha blob, quindi `checkout-index --stage=N` **non fa nulla ed esce 0** e il `git add` successivo metteva in indice il commit su cui il submodule stava **per caso** sul disco — riprodotto: «prendi il loro» lasciava l'indice sul **nostro**, riportando successo. Ora `update-index --cacheinfo 160000,<sha>,<path>` scrive l'entry e azzera le tre fasi, e il submodule viene spostato lì; se non ci si può spostare la risoluzione resta e il messaggio dice cosa manca. Merge tool disabilitato sui gitlink, i tre «nomi» mostrano i tre commit, descrizione dedicata. Verificato nei due versi col disco lasciato dalla parte opposta). Prima: `5effca45b` — **M164: il ▶ marca il branch su cui sei davvero, e sta dentro la pill** (tre difetti dietro una segnalazione: il tipo di ref era **indovinato dal nome** — barra = remoto, cifra = tag — quindi un branch locale come `feat/PO-52-activation-ladder` non prendeva il ▶ e la pill era del colore dei remoti, mentre i tipi veri erano **già caricati** da `RefreshRefContext` e non venivano consultati; il marcatore andava a **ogni** branch locale della riga di HEAD invece che a quello davvero checked-out; e «soprattutto esternamente» era una **corsa** fra il walk e l'elenco dei ref — chi arrivava secondo taceva, quindi dopo un checkout fuori dall'app le righe restavano costruite col branch vecchio. Più il ▶ spostato **dentro** la pill, dov'è nell'originale). Prima: `c59d1bf55` — **M163: la tinta dell'autore lascia il blu su cui vive la selezione** (segnalazione «poco visibile la linea selezionata»: riprodotto, e la riga della foto **non era quella selezionata** ma quella **sotto il puntatore**, resa indistinguibile. Causa mia di M162: `App.Accent` (tinta autore), `App.AccentFill` (selezione) e `App.HoverRow` erano tre blu, e sdoppiare la tinta ci ha infilato l'hover in mezzo — ΔE **2,8** in classic scuro, lo stesso colore. Ora token nuovo **`App.AuthoredTint`**, viola, a 0,14: hover a ΔE 7,1–12,1, riga semplice 10,2–11,7, striscia interna 3,2–6,0, selezione 42–89 da tutto, testo sopra 7,4:1. Verificato a schermo scuro e chiaro). Prima: `cc73616e2` — **M162: l'evidenziazione dell'autore non mangia più la striscia** (segnalazione con screenshot: una dozzina di righe consecutive diventava una lastra piatta, l'evidenziazione arrivava e la separazione fra le righe se ne andava con lei. La tinta ora si **compone** con lo sfondo alternato invece di sostituirlo — la base propria di ogni riga spostata dello stesso decimo verso l'accento, quindi la differenza sopravvive a nove decimi. Misurato: prima un colore solo `(31,39,54)` per tutto il tratto, ora `(31,39,54)`/`(40,48,65)` alternati con delta `(9,9,11)` contro il `(10,10,12)` delle righe normali). Prima: `219669815` — **M161: scegliere se anche `origin/X` divide il colore di `X`** (richiesta dell'utente subito dopo M160, che trattava ogni branch remoto come un nome a sé: un branch con `origin` indietro prendeva due colori voluto o no. Ora due risposte distinte — un branch remoto **senza** corrispondente locale tiene il suo colore sempre, perché è il branch di qualcun altro; `origin/X` accanto a `X` è lo stesso nome due volte e il confine lì separa **pushato da non pushato**, non due linee di sviluppo, quindi casella propria annidata e **spenta di default**. Un commit con entrambi conta una volta sola. Verificato a pixel: 3 colori spenta, 4 accesa, confine esattamente sui commit non pushati). Prima: `1f8002abe` — **M160: un colore nuovo a ogni nome di branch** (segnalazione dell'utente con screenshot: una colonna sola tutta rosa con dentro tre branch. Limite **strutturale** della colorazione per lane — quella di upstream e quella del port: il colore segue la corsia, e la corsia distingue solo ciò che distingue il DAG, quindi due branch non ancora divergiuti stanno nella stessa corsia e una fila dritta con tre nomi esce di un colore solo. Ora un commit su cui punta un branch inizia un colore nuovo, gli archi che arrivano da sopra tengono il loro, quindi il confine è esattamente sul commit che porta il nome. **I tag non contano**, e la «branch-ità» si legge da `IGitRef.IsHead`/`IsRemote` e non dal nome — un branch locale può avere una barra dentro, che è il caso dello screenshot. Interruttore rientrato sotto «Colour each branch…» nella pagina Revision graph, disabilitato insieme a quella. **Difetto trovato verificando**: l'interruttore non faceva nulla fino al ricaricamento, perché `BuildDisplayRows` ri-esegue la passata delle corsie solo se ha righe artificiali da innestare; ora la geometria si ricalcola esplicitamente. Verificato a pixel su Xvfb, acceso/spento a caldo nei due versi). Prima: `d02d10076` — **M159: il repository host GitHub** (era lo SKIP dichiarato più vecchio: upstream lo fa come plugin MEF sulla libreria `Git.hub`, non ripristinabile qui, quindi è lo stesso insieme di funzioni scritto contro il REST v3 e come **servizio**. Fork and clone · Create pull request · View pull requests · Add "upstream" remote nel menu GitHub, che non è più un segnaposto disabilitato; **View in GitHub** nella griglia e nel blame; le issue assegnate come template del messaggio di commit. **Il token è una password**: va all'helper di credenziali di git — il portachiavi — sotto `api.github.com` e mai in `app-settings.json`, la casella nelle Impostazioni è a sola scrittura, e siccome `git credential approve` accetta e dimentica senza helper il valore viene **riletto** prima di dire dov'è finito. Quattro scostamenti da upstream, ognuno un difetto suo: Create pull request chiede **tutti e due i capi** — quello di upstream non offre nessuna destinazione a chi possiede il repository e pusce diretto; View pull requests **si apre senza token**, perché leggere le PR di un progetto pubblico non richiede credenziali e sono disabilitate solo commentare e chiudere; il diff arriva dal media type `.diff` dell'API e non dal `diff_url` pubblico, che su un repository privato dà 404; il padre di un fork si legge alla selezione, perché GitHub riempie `parent` solo su un repository chiesto per nome. Verificato contro GitHub vero su Xvfb: **28 pull request** di `gitextensions/gitextensions` elencate senza token, diff spezzato in **14 file**, conversazione, e col token finto la frase esatta di GitHub sul token rifiutato. **Non verificata a schermo una chiamata autenticata riuscita**: non ho un token e quello dell'utente non si tocca). Prima: `887523b52` — **M158: i sei scope hotkey per controllo** (upstream lega i tasti per form/controllo e decide il focus; il port aveva solo lo scope finestra e le altre superfici confrontavano i tasti inline, quindi non erano riconfigurabili: ora `RevisionGrid`, `FileViewer`, `RevisionDiff`, `RepoObjectsTree`, `Commit`, `Stash`, coi soli comandi che il port esegue, sezione per scope in Impostazioni, la regola di precedenza di `MainWindow` che **chiede allo scope** invece di elencare tasti cablati, e le etichette dei gesti lette dalla tabella). Prima: `57b9ba887` — **M157: la selezione di una lista di file non è un cambio di scheda** (crash segnalato: `Cannot change source while update is in progress`; `SelectionChanged` **risale**, quindi la lista dentro una scheda arrivava all'handler del `TabControl` che ricaricava la scheda riassegnando `ItemsSource` sulla lista che Avalonia stava aggiornando; riprodotto, poi corretto su tutti e tre gli anelli: evento fermato nel controllo, host che guardano il `Source`, `Rebuild` che non rientra). Prima: `e6e1d669d` — **M156: script utente con gli hook Before/After** (l'ultima lacuna funzionale grossa: `scripts.json`, pagina Scripts in Impostazioni, dodici eventi agganciati ai funnel veri, più menu Tools e menu contestuale della griglia; **uno script `Before…` che fallisce FERMA l'operazione**, come upstream; **nessuno script passa da una shell** — comando + lista di argomenti, così un ref con uno spazio o un punto e virgola non diventa comandi in più). Prima: `98e88c68e` — **M153bis: l'evidenziazione dell'autore non sembra più una selezione** (usava `App.HoverRow`, il colore del puntatore, e dipingeva **ogni** riga in una repo con un autore solo: ora tinta mescolata dalla palette e si tira indietro sopra il 90% di righe combacianti). Prima: `809c859b8` — **M151-M155: le ~35 impostazioni senza consumatore, cablate davvero** (era uno SKIP dichiarato: «~35 impostazioni upstream sarebbero pulsanti finti»; l'utente ha chiesto di implementarle e ognuna ha ora un consumatore vero, con cinque pagine nuove nella finestra Impostazioni. `M151` editor del messaggio — i due limiti segnano l'eccedenza e chiedono conferma al commit, auto-wrap che spezza sull'ultimo spazio, nuovo `Views/TextRulerOverlay`; `M152` diff — righello e marcatore di fine riga presi da AvaloniaEdit, `ShowDiffForAllParents` che riusa le N sezioni di M143 con la patch del parent giusto, scroll continuo che si **arma** in fondo e salta al notch dopo; `M153` griglia — striscia, evidenziazione dell'autore e raddrizzamento erano **comportamenti senza interruttore**, grigio delle lane separato dal grigio del testo, tooltip di riga che prima non esistevano, nuovo evento `SettingsService.Changed`; `M154` stash/checkout/push — due flag «untracked» al posto di **cinque risposte cablate in disaccordo fra loro**, le due auto-pop come Ask/Always/Never, tendina a tre del push; `M155` recenti/output/ricerca rapida/font). **IL DIFETTO GROSSO TROVATO STRADA FACENDO**: ventisette superfici chiedevano la famiglia `"monospace,Consolas,Menlo"`, ma su Linux `monospace` è un **alias di fontconfig e non un nome di famiglia** — Skia non risolveva nessuna delle tre, quindi **diff, editor del messaggio, console e blame erano tutti proporzionali**; misurato con dieci «i» contro dieci «W», corretto con `Theming/AppFonts` che cerca una famiglia esistente. Prossima libera: **M156**. Prima: `bfaee4643` — Prima: `b83238a74` — **M147-M148: `git grep` dalla lista dei file, e le due view morte cablate**. Prima: `174146b3d` — Prima: `44850dc33` — **M136-M145: chiuse dieci voci della coda utente in un blocco solo** (`M136` copy path assoluto col sottomenu; `M137` impostazione «Terminal command» con `{dir}`/`{shell}`, che è il modo per usare Warp; `M138` «Fetch all» e «Fetch and prune all» in toolbar con tre glifi nuovi; `M139`-`M141` il layer `T()` esteso a **tutte** le view rimaste — ~290 stringhe in 21 file, tre subagent in parallelo su file disgiunti; `M142` `TranslationService.TPlural` (due forme, non un «(s)») col bisect come primo chiamante, palette della sintassi Classic ri-solta per la protanopia — peggior coppia da ΔE 6,45 a 24,54 — e titolo di `PushDialog` senza path; `M143` da 2 a 4 revisioni selezionate ora danno **più gruppi di diff sul merge base** come `FileStatusDiffCalculator`, con `FileStatusListView` che sa mostrare N sezioni e il clic su un file che carica la coppia del suo gruppo; `M144` pill delle note tematizzata (`App.RefNote`, viola, ΔE 48 dagli altri ref sotto simulazione) e via i due alias di `CollapseHome`; `M145` **più schede sulla stessa repository** — `RepoTabEntry.Id` GUID, «Duplicate tab» nel menu della scheda, etichette numerate `repo (1)`/`repo (2)`, migrazione dello `ui-state.json` vecchio senza perdere schede). Prossima libera: **M146**. **Tre voci verificate e chiuse senza scrivere codice** (la regola «verificare la premessa» ha evitato tre riscritture): la guardia «nothing staged» esclude già un merge commit legittimo via `MERGE_HEAD`, il discard del dialogo di commit è già multi-selezione, e il titolo è già centrato sulla finestra da M128. **Due view risultate morte**: `BranchTagPanel` e `RemotePanel`, nessuno le istanzia — da decidere se cancellarle. **Residuo aperto della vecchia nota P2**, riscritto e ridotto: manca solo la ricerca `git grep` dalla lista dei file. **Difetto minore noto**: dopo «Duplicate tab» il riquadro Commit info resta vuoto finché non si tocca la selezione. Prima: `62ac7447a` — **M135**. Prima: `a073cd787` — **M134: spinner di caricamento su griglia e albero** (nuovo `Views/BusyOverlay`: velo + arco rotante + didascalia, con **ritardo di 250 ms** perché i ricaricamenti brevi non devono produrre un tremolio; griglia: solo sopra la lista e solo sui restart, mai sugli append; albero: tutto il pannello, nascosto solo quando una passata dipinge davvero, altrimenti il passaggio di consegne fra passate superate farebbe lampeggiare il velo e ripartire il ritardo). Prossima libera: **M135**. Prima: `49841734d` — **M133: la finestra aperta ha l'icona del prodotto** (mancavano due cose, misurate con xprop: `Window.Icon` non era mai assegnata quindi `_NET_WM_ICON` non esisteva e il dock metteva l'ingranaggio, e il `.desktop` non dichiarava `StartupWMClass=GitExtensions.Avalonia`; nuovo `Theming/AppIcon`, installato come style sull'`Application` perché `Window.Icon` è una styled property e così copre ogni finestra; va reinstallato il .deb per la voce .desktop). Prossima libera: **M134**. Prima: `d7c73b0a3` — **M132: schede trascinabili e doppio clic che fissa** (il `DoubleTapped` non partiva mai perché `Sync` faceva Clear+re-add dei figli a ogni attivazione e ri-genitorare azzera lo stato di input: ora i figli si toccano solo se la sequenza cambia e il doppio clic si legge da `ClickCount`; riordino con soglia di 5 px, slot scelto sul punto medio della vicina, puntatore catturato dalla striscia, trascinare fissa la scheda come in VS Code, ordine già persistito dalla lista). Prossima libera: **M133**. Prima: `6bf91a68d` — **M131: più repository in una finestra, schede stile VS Code** (clic singolo su submodule/worktree = anteprima in corsivo che la prossima sostituisce, doppio clic la fissa, ogni altra porta apre fissato; nuovo `Views/RepoTabStrip`, `Theming/RepoTabsOption`, `UiState.OpenRepoTabs`/`ActiveRepoTab`, opzione «Repository tabs» in Appearance con le schede come default; una sola copia delle viste — la scheda porta riga selezionata e tab in basso, catturati da `_loadedTab` e non da `Active` perché la striscia ha già cambiato attivo, e la selezione è registrata con `SelectCommitWhenLoaded` perché la griglia carica in asincrono; `Ctrl+W` chiude la scheda, `Ctrl+PagSu/Giù` scorrono). Prossima libera: **M132**. Prima: `8c30911c7` — **M130: i pulsanti della finestra Stash come quelli del dialogo di commit** (Apply/Pop/Drop e i quattro pulsanti di salvataggio erano gli ultimi con la chrome di Fluent, contorno chiaro compreso: ora `BarButtonStyles.ApplyActions`, riempimento alzato e nessun bordo, solo in Modern; non `Apply`, perché non stanno su una barra). Prossima libera: **M131**. Prima: `c3008bd0b` — **M129 + due correzioni: in modern la scheda selezionata è sottolineata, e il cambio di stile non crasha più** (un template per stile ri-templatizzava i `TabItem` e Avalonia rifiutava di ri-genitorare l'header vivo: ora template unico a tre righe e il modern sposta il marcatore con un setter su `Grid.Row`; raggio degli angoli legato invece che assegnato; e `Grid.SetRow` nel template rendeva la riga un valore locale, quindi la linea restava sopra l'etichetta — ora la riga non si imposta nel template). Prossima libera: **M130**. Prima: `7a33eb988` — **M128: il menu nella barra del titolo, stile VS Code** (su X11 `ExtendClientAreaToDecorationsHint` è un no-op — sondato — mentre `SystemDecorations.None` è onorato: quindi `Views/TitleBar` disegna i pulsanti finestra e `Views/ResizeGrips` restituisce gli otto bordi; overflow «…» misurato da `MainMenu.FitTo`, mai una soglia; opzione «Title bar» in Appearance, default unificata, `UiState.TitleBar`, **indipendente** dallo stile, cambio a caldo). Prossima libera: **M129**. Prima: `3b8ee7ec0` — **M127: il pulsante della shell apre un terminale che sopravvive** (`x-terminal-emulator` qui è Warp, che rifiuta `-e` ed esce con 2 dopo che `Process.Start` è già riuscito: si riportava successo e non si provavano gli altri; ora 700 ms di grazia, uscita non-zero = fallimento e si passa al candidato dopo, lista allargata a kgx/ptyxis/tilix/terminator/mate-terminal/alacritty/kitty/foot/urxvt, lanci esterni fuori dal thread UI). Prossima libera: **M128**. Prima: `0156dd902` — **M126: un dialogo si apre sul pulsante per cui è stato aperto** (`DialogKeys.FocusOnOpen`, usato da Push e Pull; focus e non attivazione, e mai sui dialoghi che aprono su una domanda o su un'azione distruttiva). Prossima libera: **M127**. Prima: `96715fac8` — **M125: markdown nell'evidenziazione della sintassi** (`.md` cadeva in `_ => null`; passata dedicata — fence, titoli, citazioni, marcatori di lista, code span, grassetto/enfasi e la URL dei link — che riusa il bit di blocco degli altri linguaggi). Prossima libera: **M126**. Prima: `76595811c` — **M124: la storia di un file tiene rami e merge** (come l'originale: `git log --name-only --follow` per raccogliere i nomi storici, poi il walk ordinario con quei nomi come pathspec e **senza** `--follow`, così git riscrive i parent — nuovo `Services/FollowedPathService`; riabilitati Branches e View in modalità file-history, scope d'ingresso all-branches; fallback a `--follow` + `ChainFollowedHistory` per cartelle/più path/pathspec > 31000, deciso da `RevisionPage.FollowedWithoutParentRewrite`; la mappa dei nomi storici è un sottoprodotto del passo 1). Prossima libera: **M125**. Prima: `37658f73c` — **M123: pannello del diff virtualizzato su AvaloniaEdit** (nuova dipendenza `Avalonia.AvaloniaEdit` **11.3.0**, fissata alla linea 11.3; `PORTING.md` a file intero, 6559 righe: **1547 ms → 70 ms**, e lo scroll non ridisegna più tutte le righe; nuovo `Views/DiffColorizing.cs`, `DiffSyntaxHighlighter` riusato intatto; ¶ non riscrive più il testo, Ctrl+C rispetta la selezione, via i tetti dell'evidenziazione; `.deb` invariato, verificato con publish self-contained). Prossima libera: **M124**. Prima: `af9f0301a` — **M122: lo stile classico ritrova il bitmap della lente** (`Icons.ClassicNameOf`: «Search» → `Preview.png`, consultato da `GlyphSource.Draw` prima dell'asset loader; l'avviso `[IconLoader] icon 'Search' did not resolve` era il classico che chiedeva un PNG inesistente). Prossima libera: **M123**. Prima: `c8ffa023c` — **M121: anche la toolbar di File History è piatta** (i suoi quattro pulsanti erano l'ultima striscia incorniciata: ora `toolbtn` di `BarButtonStyles`). Prossima libera: **M122**. Prima: `3f6a28c1b` — **M120: la finestra File History disposta come `FormFileHistory`** (toolbar in cima, riga di stato collassata quando non ha nulla da dire, una sola sonda sul blob invece di due, pulsante Git command log, splitter 1*/4/3* con `MinHeight` **sulla riga**, Ctrl+Tab fra le schede saltando quelle disabilitate). Prossima libera: **M121**. Prima: `fc24d6e8e` — **M119: una toolbar sola, un filtro solo** (`ToolStripMain` era già fedele; via la mezza copia del filtro in alto — il menu «All branches» mentiva sull'ambito — e via l'etichetta repo/branch in fondo, entrambe assenti in `FormBrowse`; aggiunte le scorciatoie Pull-merge/Pull-rebase di `InsertFetchPullShortcuts` e i suffissi di hotkey nei tooltip; `Ctrl+E` va alla casella della griglia; la striscia ora entra senza overflow). Prossima libera: **M120**. Prima: `1582e4128` — **M118: il grafo disegnato come nell'originale** (palette `GraphBranch1..7` a sette colori, metriche di `GraphRenderer`, nodo quadrato per le righe con ref e anello su HEAD, diagonali a Bézier, `StraightenLaneShifts` che ricuce le due metà di un arco in una sola diagonale nodo-a-nodo, Author 130 / Commit ID 64). Prossima libera: **M119**. Prima: `c92d4252b` — **M117: la lista dei file cambiati dice di che confronto è** (intestazione «(N)  Diff with A `<sha>`: `<subject>`» come `FileStatusWithDescription.Summary` dell'originale: `SetFiles(rows, summary)` la disegna come gruppo pieghevole sopra gli altri; la revisione è nominata sul thread di background del load via `DiffService.DescribeRevision`/`FirstParentOf`; il commit radice resta senza intestazione). Prossima libera: **M118**. Prima: `2924ccc7e` — **M116: una sola linea nella storia di un file, e il diff di una selezione multipla** (`--follow` NON fa riscrivere i parent da git, quindi il grafo del file era una scaletta di monconi: `RevisionService.ChainFollowedHistory` ri-collega ogni riga a quella sotto, toccando solo `GraphParents`; da tre commit selezionati in su si mostrava un commit solo → `RangeEnds` prende i due estremi, con dedup dell'annuncio perché un Ctrl+clic alza `SelectionChanged` due volte; la finestra File History ora inoltra `RangeSelected` e confronta gli estremi con il suo file preselezionato; la preselezione viaggia col load e non più in un campo). Prossima libera: **M117**. Prima: `4384fbc1c` — **M115: la barra della griglia e le sue tendine parlano la lingua dell'app** (pulsanti della seconda riga piatti come toolbar e dialogo di commit — classe `toolbtn`, contorno barattato con l'hover, compromesso 1.4.11 dichiarato; le sei tendine `Flyout` perdono il `Border` interno e diventano una carta sola sul `FlyoutPresenter` — baseline, modern aggiunge angolo e ombra; i comandi dentro le carte prendono la forma di voce di menu con `BarButtonStyles.ApplyMenus`, installata sull'`Application` perché una pop-up root non vede le styles della vista). Prossima libera: **M116**. Prima: `571b87864` — **M114: colonne della griglia ridimensionabili** (presa da 6px sul bordo sinistro di autore/data/commit id — lo stesso insieme dell'originale; subject = colonna che assorbe, pavimenti 40/120px, righe ri-templatizzate solo al rilascio, larghezze persistite in `ViewPrefs.GridColumns` — cosa che l'originale **non** fa). Prossima libera: **M115**. Prima: `10614aa03` — **M113: la storia di un file è una finestra a sé (`Views/FileHistoryWindow`), come `FormFileHistory`** (griglia del file + le quattro schede Commit/Diff/View/Blame, caricamento pigro, nome storico su rename; nuova `FileContentView`, `DiffView.ShowCommit` con preselezione del file; **la scheda "File history" in basso è stata rimossa** e tutti i punti d'ingresso aprono la finestra). Prossima libera: **M114**. Prima: `0cba58207` — **M112: rotella a tre righe per scatto e scorrimento al passaggio del puntatore sui chevron** (`Theming/MenuScrolling`, attached property messa da una style su tendine, menu contestuali e flyout). Prossima libera: **M113**. Prima: `6926e58fb` — **M111: la tendina si ferma al bordo inferiore della finestra e scorre** (`App.MenuMaxHeight` pubblicata da `MainWindow` e letta dalla carta del menu come dynamic resource). Prossima libera: **M112**. Prima: `769b6fdcd` — **M110: i menu a tendina non si aprono più sopra la barra dei menu** (niente FlipY/SlideY sui popup di `MenuItem`, solo ResizeY: la carta si accorcia e scorre). Prossima libera: **M111**. Prima: `7fcf2204e` — **M109: pulsanti azione pieni (niente contorno) nel dialogo di commit + menu con la forma di VS Code** (pillola arrotondata rientrata, separatori a tutta larghezza, carta del popup arrotondata con ombra; tutto solo in Modern). Prossima libera: **M110**. Prima: `5c5d76c7c` — **M108: allineato a `upstream/master` (merge `736a6ed6d`, 11 commit, zero conflitti) + tre fix riportati a mano** (nome del branch del worktree normalizzato con `-b`, offerta di passare al worktree appena creato, avviso "perdi commit" nel reset di un altro branch via `merge-base --is-ancestor`; più `Theming/MenuText` che raddoppia l'underscore negli header dei menu). Prossima libera: **M109**. Prima: `d5f4aca02` — **M107: via le scatole dal dialogo di commit** (contorni dei pannelli solo in Classic via `StyleDensity.PaneOutline`, liste su `App.Panel`, splitter trasparenti, toolbar dei pannelli con i bottoni piatti di `Theming/BarButtonStyles`: 14139 → 1383 pixel di bordo). Prima: `4099dce58` — **M106: tema di sistema + worktree verde, submodule bicolore, barra inferiore colorata** (`UiState.Theme` accetta **System**, che è anche il nuovo default: risolto e inseguito da `App/Theming/SystemTheme.cs` via `IPlatformSettings`/portal XDG, con seme `SystemThemeSeen` per togliere il flash bianco all'avvio e reconcile a 1s; worktree da ciano a verde; glifo submodule bicolore via la nuova tabella `Icons.Parts` disegnata parte per parte da `GlyphSource`; accenti per tutti gli otto tab della striscia inferiore). Prossima libera: **M107**. Prima: `321e09a3f` — **M105: build a zero warning** (34 → 0, risolti non zittiti: `App/Async.cs` per gli `async void`/lambda async e per l'idioma `ContinueWith`+`t.Result`, remoti letti in modo sincrono, join delle traduzioni senza attesa su task, snapshot chiesti alla cache, `MaintenanceDialog` irraggiungibile eliminato; una modifica al sorgente condiviso in `Executable.cs`). Prossima libera: **M106**. Prima: `56772bcc3` — **M104: le ultime nove icone senza glifo** (i quattro del log dell'utente + i sei verdetti GPG e `FunnelPencil` trovati con un audit statico; più il fix della `Source` in `SetSubmoduleNavigation`). Un run headless in stile Modern non stampa **nessuna** riga `[IconLoader]`. Prossima libera: **M105**. Prima: `7545fd235` — **M103: icone moderne colorate per ruolo, con interruttore in Appearance**. Prima: `9a894c461` — **M102: fascia del chevron nell'albero**. Prima: `90e03397d` — **M101: raggruppamento delle liste del dialogo di commit, overflow della toolbar, icona del branch, messaggio dai submodule**. Prossima libera: **M102**. Prima: `77ed2ebdc` — **M100: struttura interna dei pannelli del dialogo di commit**. Prossima libera: **M101**. Prima: `9e3ed0165` — **M99: icone del dialogo di commit**. Prossima libera: **M100**. Prima: `d8c92123a` — **M98: dialogo di commit allineato a FormCommit** (`479355e1c`, `42c9ac1a8`, `d8c92123a`). Prossima libera: **M99**. Prima: `93ce713b0` — **M97: una sola riga selezionata nell'albero + finestra dello stash** (`c19f74ee7`, `93ce713b0`). Prossima libera: **M98**. Prima di M97: `1a8abcd7c` — **M90: doppio clic Submodules usa il target reale e mostra feedback immediato**. Commit M90: `1e9a0bf5b`, `1a8abcd7c`. M89: `32e981301`…`3b995372d`. Prossima libera: **M97** · `d582225f4` (**M96: densità della chrome, solo Modern**) · `35a98fd3d` (**M95: chrome moderna piatta**) · `7fe3726a8` (**M94: icone complete, App.Link, contorni a 3:1, focus non tagliato**) · `5c5a03a64` (**M93: hover della riga + fine del flash bianco**) · `044ce45dc` (**M92: larghezze dei pannelli**) · `363961635` (**M91: submodule nidificati**). |
| Build | `Errori: 0`, **`Avvisi: 0`** su tutti gli otto progetti da M105 (confermato a M108, anche dopo il merge da upstream) (prima erano 34 fra VSTHRD e CS). Harness navigation snapshot: PASS; hierarchy M87/M91: PASS, 7 nodi, inclusi ciclo e linked worktree. **I progetti sotto `Tests/` sono ora otto** (i tre più recenti sono banchi di regressione: `InlineDiffRegression`, `CommandPaletteRegression` con `PASS: 10037 casi` a M197, `ViewPrefsRegression` con `PASS: 41 casi` a M204 — risultati **riportati dalle rispettive milestone**, non rimisurati qui). Nessun banco è agganciato alla soluzione o a un workflow di CI: si lanciano **a mano**. |
...
| Push su remote | **NON allineato al 14-08-2026**: `origin/linux-avalonia-port` è fermo a `8cea6b14e` (M202), mentre `HEAD` è `7b84e9824`. `git rev-list --count origin/linux-avalonia-port..HEAD` dà **5**: `aebffd9e8` (M203), `00175a9b2` (documentazione M196–M203), `99e1c74b4` (M204), `2c5bcdf26` (M205), `7b84e9824` (M206). Il push va rifatto (verificare sempre con lo stesso `rev-list --count`, che deve tornare a 0). Il push lo esegue l'utente, mai il loop. Portachiavi: se vuoto, il primo push chiede le credenziali **una volta** (username `daryda9` + PAT), poi `git credential approve` le salva in libsecret |
...
- **SKIP consapevoli, non riaprire senza una ragione nuova** — ne resta **uno**: la colonna
