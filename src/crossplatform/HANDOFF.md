...
| Push su remote | **ALLINEATO al 24-08-2026**: `origin/linux-avalonia-port` e `HEAD` sono entrambi a `104a8c861`; `git rev-list --count origin/linux-avalonia-port..HEAD` dà **0**. Il push lo esegue l'utente, mai il loop: verificare sempre con quel `rev-list --count`, che deve tornare a 0. Portachiavi: se vuoto, il primo push chiede le credenziali **una volta** (username `daryda9` + PAT), poi `git credential approve` le salva in libsecret. **La CI gira su questo push** (`.github/workflows/crossplatform-build.yml`): il primo giro, sul push precedente, era rosso per lo stallo chiuso in M215 — il risultato di questo va guardato. I log dei run **non** si leggono senza token (da anonimo l'API risponde `403 Must have admin rights`, e `gh` non è installato) |
...
> 1. **Guardare il risultato della CI** su `104a8c861` — è il primo giro dopo il fix del M215, ed è
>    l'unica verifica che manca a quel lavoro. I log vogliono un token (vedi §1).
...
>    o path non ASCII, selettore di commit cablato su **uno** dei quattro campi che upstream serve.
...
**Stato al 24-08-2026.** `HEAD` = `origin/linux-avalonia-port` = `104a8c861`. **Nessuna coda utente
aperta**: le tre voci del 31/07/2026 sono chiuse da tempo (M75, M149bis). Prossima milestone libera:
**M218**. Ultimo giro: README rifatto con dodici schermate (M215 → M217), il fix del `.git` finto
(M216) e la verifica una-per-una delle differenze dall'originale (M217).
...
Le tre cose che un giro nuovo può prendere, in ordine di costo:

1. **Il risultato della CI su `104a8c861`** — primo giro dopo il fix del M215. Costa nulla e chiude
   quel lavoro. I log vogliono un token: da anonimo l'API risponde `403`.
2. **Un residuo misurato** dalla lista di `ROADMAP.md` → «Residui ancora aperti»: il più utile è il
   **selettore di commit sugli altri tre campi** che upstream serve (cherry-pick, archivio, confronto
   fra due revisioni), perché il dialogo esiste già (`Views/ChooseCommitDialog`) ed è cablatura.
3. **Una delle due voci grosse, INEDITE, e solo se l'utente le chiede**: **undo timeline sul reflog**
...
1. git -C /home/dario/git_ext_mod rev-parse --short HEAD  (atteso 104a8c861 o piu' avanti; se diverso,
...
1. guardare il risultato della CI sul push corrente (primo giro dopo il fix M215)
2. un residuo dalla lista di ROADMAP.md — il piu' pronto e' offrire Views/ChooseCommitDialog anche a
   cherry-pick, archivio e confronto fra due revisioni (upstream lo fa su quattro campi, qui e' su uno)
...
Quando consegni: aggiorna PORTING.md (nuova milestone, la prossima libera e' M218), ROADMAP.md se
