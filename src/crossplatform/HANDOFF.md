...
> 0. **Un banco per gli asset**, che è la lezione dei tre difetti di questi due giorni (icona di
>    finestra, icona nella barra, illustrazione dei dialoghi): qui **una risorsa mancante non fa
>    rumore** — `AssetLoader` risponde «non c'è» e i chiamanti disegnano niente. Un banco che chieda
>    `AssetLoader.Exists` per ogni nome che l'app carica (le sette immagini di aiuto, il marchio, i
>    glifi) e fallisca **per nome** costa poco e chiude la famiglia. Da fare.
