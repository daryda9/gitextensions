...
| Parità voci UI/funzionali | la **"Coda round 9"** in `PORTING.md` (la misura buona, area per area) è **ESAURITA**: zero voci `[ ]`, zero `[~]`. Restano gli SKIP dichiarati — repository-host GitHub, colonna build status, i 6 scope hotkey oltre a `FormBrowse`. **Gli script utente NON sono più uno SKIP: chiusi in M156.** **Le ~35 impostazioni senza consumatore NON sono più uno SKIP: chiuse in M151–M155** |
