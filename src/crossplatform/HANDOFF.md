...
| Push su remote | **NON allineato al 14-08-2026**: `origin/linux-avalonia-port` è fermo a `29d037eea` (documentazione M204–M206), mentre `HEAD` è `c5152dd43`. `git rev-list --count origin/linux-avalonia-port..HEAD` dà **4**: `418036239` (M207), `df83ed36a` (M208), `76d0504c4` (M209), `c5152dd43` (M210) — più la documentazione di questo giro. Il push va rifatto (verificare sempre con lo stesso `rev-list --count`, che deve tornare a 0). Il push lo esegue l'utente, mai il loop. Portachiavi: se vuoto, il primo push chiede le credenziali **una volta** (username `daryda9` + PAT), poi `git credential approve` le salva in libsecret |
