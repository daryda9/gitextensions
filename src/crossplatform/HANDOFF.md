...
| Push su remote | **NON allineato al 17-08-2026**: `origin/linux-avalonia-port` è a `eddbbfb6f` (documentazione M214), mentre `HEAD` è `c7a17147a`. `git rev-list --count origin/linux-avalonia-port..HEAD` dà **4**: `1b9d40c66` (M215), `f2db50dee` e `b3b0b21bc` (README con le schermate), `c7a17147a` (M216) — più la documentazione di questo giro. **Il push di `eddbbfb6f` ha fatto girare la CI per la prima volta e il run è rosso** (`navigation-snapshot` appeso, vedi M215): il fix sta in questo commit, quindi il prossimo push è anche la verifica che il workflow torni verde. Log della CI: le API li danno solo con un token (`403 Must have admin rights` da anonimo), quindi vanno letti dal browser o con un PAT. Il push va rifatto (verificare sempre con lo stesso `rev-list --count`, che deve tornare a 0). Il push lo esegue l'utente, mai il loop. Portachiavi: se vuoto, il primo push chiede le credenziali **una volta** (username `daryda9` + PAT), poi `git credential approve` le salva in libsecret |
