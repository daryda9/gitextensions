...
| HEAD al momento dell'handoff | **M228 (2026-09-04)** — chiesto dall'uso: il click destro su una cartella nel dialogo di commit ha un menu suo, con `Stage` / `Unstage` / `Discard changes` / **`Stash <cartella>`** della cartella, più le tre voci che l'upstream offre lì (`Select all`, `Collapse all`, `Expand all`) e `Copy path`. Prima faceva **peggio di niente**: piegava la cartella (il gestore in fase di tunnelling trattava ogni bottone allo stesso modo) e apriva il menu dei file, che agisce sulla **selezione** — quindi le voci valevano per il file selezionato altrove, «Reset file changes» compreso. Misurato sull'upstream: sull'intestazione offre solo le quattro voci d'albero e **nessuna azione git** (lì si fa «seleziona tutto» e poi il menu dei file). Tre fatti su git misurati prima di appoggiarci il menu: un pathspec di **directory** lascia indietro i non tracciati in silenzio; un **file** non tracciato senza `-u` fa **fallire tutto**; e con un merge irrisolto `git stash push` rifiuta **qualunque** pathspec, anche estraneo al conflitto (perciò la voce è spenta, e il servizio riceve i file e non la cartella, per non stashare più di quello che il pannello mostra). Il punto fragile: «questa cartella» il menu lo ricava **all'indietro** dall'intestazione, perché una cartella piegata non mostra i suoi file — due calcoli indipendenti dello stesso insieme, quindi `GroupKey` è ora un metodo della classe usato da entrambi e il banco li incrocia col conteggio che l'intestazione stampa. `Tests/FileListRegression` a **67 casi**; il banco ha bocciato due cose del codice appena scritto (il mio `SelectFolder` leggeva dal `ListBox`, e una mia asserzione era falsa). Verificato a schermo gesto per gesto. Prima: **M226 + M227 (2026-09-03)** — segnalati dall'uso, entrambi nel dialogo di commit raggruppato per cartelle. **M226**: piegare una cartella non la nascondeva, **la buttava**. Il raggruppamento era giusto; il pane non aveva altra memoria delle sue righe che gli `Items` del `ListBox`, e un gruppo piegato tiene il suo sottoalbero fuori dagli `Items` per costruzione — quindi la ricostruzione dopo il click partiva da ciò che era rimasto a schermo. La stessa lettura decideva il contatore, **su cosa agiscono `Stage all` / `Unstage all`** (solo i file visibili, in silenzio), il conteggio in barra e i pulsanti di reset; undici punti leggevano `Items`, che con un raggruppamento attivo **contengono anche le intestazioni**. Ora `FileListPane.Rows` è la lista autoritativa e `SetPaneRows` l'unico modo di darle al pane. Trovati leggendo, nello stesso giro: il pulsante ☰ piatto/albero non faceva niente di visibile (assegnava il campo e basta), e la selezione automatica della lista in stage prendeva `Items[0]`, che raggruppando è un'intestazione. **Il raggruppamento ora si ricorda** (`ViewPrefs.FileList`, tre scelte: la condivisa di sessione e una per ciascuna lista del dialogo; l'upstream ne tiene **una sola** globale, `AppSettings.DiffListSorting` diffusa dal singleton `DiffListSortService`, ma qui le due liste hanno una barra ciascuna e possono già stare a cartelle da un lato e piatte dall'altro). **M227**: `./run.sh` scegliva il binario con `find bin -name GitNext | head -1`, e sotto `bin/` ce ne sono **quattordici** (ogni banco che referenzia l'app ne riceve una copia) in ordine di filesystem — instabile fra due invocazioni; `run.sh` compila solo il progetto dell'app, quindi quelle copie sono vecchie quanto l'ultima build di quel banco. **Misurato**: un `GitNext.dll` di oggi e quattordici del 1 settembre. È per questo che M226, corretto e verde, si è ripresentato identico all'utente: dimostrato guidando lo stesso gesto sul binario del 1 settembre sotto Xvfb (la cartella riaperta scompare) e su quello di oggi (no). Ora il percorso lo dice MSBuild (`-getProperty:TargetPath`), **dopo** la build e non al suo posto: `-getProperty` valuta, non compila, e **restituisce 0 anche col codice rotto**. Undicesimo banco: `Tests/FileListRegression` (`DiffFileListBuilder`, finora scoperto; il lint sui sorgenti del dialogo; la preferenza col ponte fra i due enum), provato reintroducendo il difetto **tre** volte. Verificato anche nell'app sotto Xvfb, non solo nei banchi. **Undici** banchi verdi. Prossima libera: **M228**. Prima: `9d23153c5` — **M225: un banco che fallisce col nome quando un asset non si risolve** (2026-09-01). Chiude la famiglia dei tre difetti di fine agosto (icona di finestra, icona nella barra, illustrazioni dei dialoghi): **qui una risorsa mancante non fa rumore**, e li ha trovati tutti l'uso. `Tests/AssetNamesRegression` gira **senza display e senza avviare Avalonia** — `Avalonia.Platform.StandardAssetLoader` è pubblica e risolve gli `avares:` da sola. Sei gruppi, due dei quali **leggono il codice** (le `HelpImageSpec` per reflection, i letterali di `IconLoader` e gli host `avares:` dai sorgenti), più marchio, bitmap Classic e un pavimento sul set imbarcato. Tre casi negativi sempre in esecuzione: non vacuo per costruzione. Provato reintroducendo il difetto due volte (nome vecchio dell'assembly: cinque fallimenti; refuso `Assets/Helps/`: quattro, con l'URI che il pannello avrebbe chiesto). Se non trova i sorgenti **fallisce**, non salta. **Dieci** banchi verdi. Prossima libera: **M226**. Prima: `ceb7619b0` — **M224: la rinomina aveva spento l'illustrazione dei dialoghi di merge e rebase** (2026-08-27). Segnalato dall'uso. L'host di un URI `avares:` è il nome dell'assembly, e `HelpImagePanel` lo aveva scritto a mano: dopo la rinomina `AssetLoader.Exists` diceva no e il pannello, che tratta un asset mancante come «non disegnare», lasciava la colonna vuota **in silenzio** — né build né banchi se ne accorgono. Cinque letterali col nome vecchio: quattro innocui per fortuna (base di `StyleInclude`), uno fatale. Ora c'è `Theming/AssetUri`, unica base chiesta all'assembly, usata da tutti. **Terzo difetto della stessa famiglia in due giorni**: serve un banco sugli asset (§4 punto 0). Prossima libera: **M225**. Prima: `69c8f8ae1` — **M223bis: l'icona di finestra non arrivava a nessuna finestra** (2026-08-26): lo stile usava `OfType<Window>()`, che in Avalonia è il tipo **esatto**, e tutte le 47 finestre derivano da `ZoomWindow` — installato, mai combaciato, `Window.Icon` mai assegnata dal M133. Ora nel costruttore di `ZoomWindow`. Chiusa dall'utente con `xprop`: `Icon (128 x 128)`. Lezione: uno `Style` che non combacia **non fallisce**, tace. Prima: `24531a560` — **M223: il port si chiama gitNext e porta un marchio suo** (2026-08-26). Nome deciso dall'utente. Sette forme: `gitNext` per gli umani, `GitNext` come `AssemblyName` — **che è anche il `WM_CLASS`**, quindi deve combaciare con `StartupWMClass` in `packaging/gitnext.desktop` — e `gitnext` per comando, pacchetto, icona e `/opt/gitnext`. Namespace **invariato** (188 file per zero effetto). L'About non dice più «proudly presented by the Git Extensions team»: sotto un nome proprio quella frase attribuiva questa UI a upstream e nascondeva che il core git è loro. Marchio: SVG in `App/Assets/Icons/gitNext.svg`, un PNG per misura in `packaging/icons/`, il `.deb` le installa tutte e otto; il wordmark largo di upstream è ritirato (quel bitmap *è* la scritta vecchia) e con lui la toppa del tema chiaro. **Due scoperte**: i letterali `avares://GitExtensions.Avalonia/…` sopravvivevano solo per il fallback di Avalonia sull'assembly d'ingresso, e l'unico asset col nome nuovo NON passava da lì (icona di finestra sparita in silenzio) — ora il nome si chiede all'assembly; e il core mette le impostazioni in una cartella col nome del prodotto, quindi si è spostata: il vecchio `GitExtensions.settings`, committato per sbaglio, è fuori dall'indice e le due cartelle sono in `.gitignore`. **Non verificato**: `_NET_WM_ICON`, assente su questo Xvfb anche per la build pre-rinomina (provata in worktree) e senza window manager installato — pari al baseline, da rifare su un desktop vero. Prossima libera: **M224**. |
| Build | `Errori: 0`, **`Avvisi: 0`** su tutti gli otto progetti da M105 (confermato a M108, anche dopo il merge da upstream) (prima erano 34 fra VSTHRD e CS). Harness navigation snapshot: PASS; hierarchy M87/M91: PASS, 7 nodi, inclusi ciclo e linked worktree. **I progetti sotto `Tests/` sono ora dodici** (i sette banchi di regressione sono `InlineDiffRegression`, `CommandPaletteRegression` con `PASS: 10037 casi`, `ViewPrefsRegression` con `PASS: 41 casi`, `SettingsStoresRegression` con `PASS: 41 casi`, `ImageIntegrityRegression` con `PASS: 124 casi` `SyntaxTokenizeRegression` con `PASS: 7507 casi` (M221) e `MergeResolveRegression` con `PASS: 24 casi` (M222). **Da M211 non si lanciano più a mano**: `Tests/run-all.sh` costruisce la soluzione e gira i **nove** deterministici in ~20 s (`ALL GREEN: 9 harnesses` confermato a M222), e la CI li lancia sui path che toccano il port. Fuori dal runner, con la ragione scritta: `AnimProbe` e `ChromeProbe` (vogliono uno schermo), `Perf` (misura, non verdetto). |
...
> ### ►► DOPO LA RINOMINA (27-08-2026) — cosa resta di quel lavoro
> 0. ~~Un banco per gli asset~~ — **FATTO (M225, `9d23153c5`)**: `Tests/AssetNamesRegression`, decimo
>    banco, nel runner. Copre base, radice del pannello, diagrammi (per reflection), letterali di
>    `IconLoader` e host `avares:` letti dai sorgenti, marchio, bitmap Classic. Non vacuo per
>    costruzione, provato reintroducendo il difetto due volte. **Resta scoperto**: i glifi vettoriali
>    di `Icons.Data` — un `Data` malformato non è un asset mancante, è una `Geometry` che non si
>    disegna, e `Geometry.Parse` vuole la piattaforma di rendering inizializzata. Stessa famiglia,
>    altro meccanismo.
> 0bis. ~~Le liste di file raggruppate~~ — **FATTO (M226)**: `Tests/FileListRegression`, undicesimo
>    banco. Copre `DiffFileListBuilder` (che nessun banco toccava), il lint sui sorgenti del dialogo di
>    commit e la preferenza ricordata. **Resta fuori**, e vale saperlo: la logica di raggruppamento del
>    dialogo di commit è una **seconda implementazione** (`CommitDialog.BuildItems`, con un enum suo)
>    e non si può costruire senza display, quindi è coperta da un lint sull'invariante che ha rotto —
>    non dalle sue asserzioni. Unificarla con `DiffFileListBuilder` è il lavoro che la renderebbe
>    verificabile davvero.
> 1. ~~`_NET_WM_ICON` da verificare~~ — **CHIUSA (M223bis, `69c8f8ae1`)**: l'icona non arrivava a
>    nessuna finestra perché lo stile usava `OfType<Window>()`, che in Avalonia è il tipo **esatto**, e
>    qui tutte le 47 finestre derivano da `ZoomWindow`. Ora si assegna nel costruttore di `ZoomWindow`.
>    Verificato dall'utente sul suo desktop: `xprop _NET_WM_ICON` → `Icon (128 x 128)`, prima assente.
>    **Resta da vedere** se la barra delle app mostra il marchio: la proprietà c'è, ma da `./run.sh`
>    non esiste nessun `.desktop` installato con cui la shell possa associare la finestra al lanciatore
>    — per quello serve un giro di `packaging/build-deb.sh` e l'installazione (punto 3).
> 2. **Due schermate del README** (`main-window.png`, `repository-tabs.png`) mostrano ancora il nome
>    vecchio nella barra del titolo. Da rifare con la ricetta del M215-M217: repo usa-e-getta sotto
>    `/tmp` con submodule e worktree, Xvfb, XTEST, identità finta.
> 3. **Il `.deb` non è stato costruito** dopo la rinomina: `packaging/build-deb.sh` è stato riscritto
>    (nomi, `/opt/gitnext`, otto misure d'icona) ma non eseguito, perché serve `dpkg-deb` e un publish
>    self-contained. Da lanciare una volta e installare per vedere l'icona nel dock.
> 4. **Il logo a 16 px impasta** le due tinte: sagoma sì, frecce no. Vale una variante semplificata per
>    le misure piccole (16/24), non un logo nuovo.
>
...
**Stato al 24-08-2026 (sera).** `HEAD` locale avanti di M218–M221 + docs su `origin` (che è a
`104a8c861`): **il push spetta all'utente**. Nessuna coda utente aperta. Prossima milestone libera:
**M223**. Ultimo giro: CI verde su `104a8c861` e su `83b67a1cf` (fix M215 confermato), il selettore
...
prendere) — e il **freeze da `@`** diagnosticato su un processo vivo e chiuso con l'ottavo banco
(M221: `IsWordStart`/`IsWordChar` asimmetrici sul ramo identificatore, thread UI in loop dentro il
measure di AvaloniaEdit).
...
1. git -C /home/dario/git_ext_mod rev-parse --short HEAD  (atteso 82b0e901c o piu' avanti; se diverso,
...
- il prodotto e' gitNext: dll GitNext.dll, comando gitnext, WM_CLASS GitNext (deve combaciare con
  StartupWMClass in packaging/gitnext.desktop). Namespace ancora GitExtensions.Avalonia: NON rinominarlo
- l'app scrive le sue impostazioni in una cartella col nome del prodotto accanto ai sorgenti: e' in
  .gitignore dal M223, non committarla
...
Quando consegni: aggiorna PORTING.md (nuova milestone, la prossima libera e' M223), ROADMAP.md se
