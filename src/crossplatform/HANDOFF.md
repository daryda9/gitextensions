...
| Push su remote | **allineato all'11-08-2026**: `origin/linux-avalonia-port` == `HEAD` (`e26ab1943`), fino a M157 compreso (verificare sempre con `git rev-list --count origin/linux-avalonia-port..HEAD`, che deve dare 0). Il push lo esegue l'utente, mai il loop. Portachiavi: se vuoto, il primo push chiede le credenziali **una volta** (username `daryda9` + PAT), poi `git credential approve` le salva in libsecret |
